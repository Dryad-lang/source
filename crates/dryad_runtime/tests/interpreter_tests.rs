...
mod interpreter_tests {
    ...
    fn test_eval_division_by_zero() {
        ...
        match result.unwrap_err() {
            DryadError::Runtime(3007, _) => {}, // E3007 - Divisão por zero
    ...
    fn test_invalid_subtraction_types() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            DryadError::Runtime(3005, _) => {}, // Operação '-' só é válida para números
    ...
    fn test_invalid_multiplication_types() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            DryadError::Runtime(3006, _) => {}, // Operação '*' só é válida para números
            _ => panic!("Erro esperado de tipo"),
        }
    }
    ...
    fn test_invalid_comparison_types() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            DryadError::Runtime(3009, _) => {}, // Comparação só é válida para números
            _ => panic!("Erro esperado de tipo"),
        }
    }
