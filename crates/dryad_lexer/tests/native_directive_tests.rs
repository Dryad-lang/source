...
fn test_tokenize_simple_native_directive() {
    ...
    assert_eq!(token, Token::NativeDirective("console_io".to_string()));
...
fn test_tokenize_multiple_native_directives() {
    ...
    assert_eq!(token1, Token::NativeDirective("console_io".to_string()));
    
    let token2 = lexer.next_token().unwrap();
    assert_eq!(token2, Token::NativeDirective("file_io".to_string()));
    
    let token3 = lexer.next_token().unwrap();
    assert_eq!(token3, Token::NativeDirective("debug".to_string()));
...
fn test_native_directive_with_underscores() {
    ...
    let token = lexer.next_token().unwrap();
    assert_eq!(token, Token::NativeDirective("terminal_ansi".to_string()));
}
...
fn test_native_directive_with_numbers() {
    ...
    let token = lexer.next_token().unwrap();
    assert_eq!(token, Token::NativeDirective("module123".to_string()));
}
...
fn test_native_directive_mixed_with_code() {
    ...
    let token2 = lexer.next_token().unwrap();
    assert_eq!(token2, Token::Keyword("let".to_string()));
    
    let token3 = lexer.next_token().unwrap();
    assert_eq!(token3, Token::Identifier("x".to_string()));
    
    let token4 = lexer.next_token().unwrap();
    assert_eq!(token4, Token::Symbol('='));
    
    let token5 = lexer.next_token().unwrap();
    assert_eq!(token5, Token::Number(5.0));
    
    let token6 = lexer.next_token().unwrap();
    assert_eq!(token6, Token::Symbol(';'));
    
    let token7 = lexer.next_token().unwrap();
    assert_eq!(token7, Token::NativeDirective("debug".to_string()));
}
...
fn test_native_directive_in_expression() {
    ...
    let token2 = lexer.next_token().unwrap();
    assert_eq!(token2, Token::Number(5.0));
    
    let token3 = lexer.next_token().unwrap();
    assert_eq!(token3, Token::Operator("+".to_string()));
    
    let token4 = lexer.next_token().unwrap();
    assert_eq!(token4, Token::Number(3.0));
}
