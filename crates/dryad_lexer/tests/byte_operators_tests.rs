...
mod byte_operators_tests {
    ...
    fn test_tokenize_binary_number() {
        ...
        assert_eq!(token, Token::Number(10.0));
    }
    ...
    fn test_binary_numbers_various_values() {
        ...
        for (input, expected) in test_cases {
            ...
            let token = lexer.next_token().unwrap();
            assert_eq!(token, Token::Number(expected), "Failed for input: {}", input);
        }
    }
...
"#;
    ...
    fn test_byte_numbers_with_operations() {
        ...
        assert!(matches!(tokens[0], Token::Number(10.0))); // 0b1010
        assert!(matches!(tokens[1], Token::Operator(ref op) if op == "+")); 
        assert!(matches!(tokens[2], Token::Number(10.0))); // 0o12
        assert!(matches!(tokens[3], Token::Operator(ref op) if op == "-"));
        assert!(matches!(tokens[4], Token::Number(10.0))); // 0xA
    }
}
