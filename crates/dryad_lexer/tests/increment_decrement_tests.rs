...
mod increment_decrement_tests {
    ...
    fn test_differentiate_increment_from_plus() {
        ...
        assert_eq!(token1, Token::Operator("+".to_string()));
        
        let token2 = lexer.next_token().unwrap(); // espaço é ignorado
        assert_eq!(token2, Token::Operator("++".to_string()));
    }
    ...
    fn test_increment_decrement_in_code() {
        ...
        let increment_found = tokens.iter().any(|t| matches!(t, Token::Operator(op) if op == "++"));
        let decrement_found = tokens.iter().any(|t| matches!(t, Token::Operator(op) if op == "--"));
...
"#;
        ...
        let let_found = tokens.iter().any(|t| matches!(t, Token::Keyword(kw) if kw == "let"));
        let identifier_found = tokens.iter().any(|t| matches!(t, Token::Identifier(id) if id == "contador"));
        let increment_found = tokens.iter().any(|t| matches!(t, Token::Operator(op) if op == "++"));
        let decrement_found = tokens.iter().any(|t| matches!(t, Token::Operator(op) if op == "--"));
