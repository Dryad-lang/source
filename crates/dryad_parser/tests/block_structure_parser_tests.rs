...
fn test_parse_single_statement_block() {
    ...
    if let Stmt::Block(statements) = &program.statements[0] {
        ...
        assert!(matches!(statements[0], Stmt::VarDeclaration(_, _)));
    } else {
        panic!("Expected Block statement, got {:?}", program.statements[0]);
    }
}
...
fn test_parse_multiple_statements_block() {
    ...
    if let Stmt::Block(statements) = &program.statements[0] {
        ...
        assert!(matches!(statements[0], Stmt::VarDeclaration(_, _)));
        assert!(matches!(statements[1], Stmt::VarDeclaration(_, _)));
        assert!(matches!(statements[2], Stmt::VarDeclaration(_, _)));
    } else {
        panic!("Expected Block statement, got {:?}", program.statements[0]);
    }
}
...
fn test_parse_nested_blocks() {
    ...
    if let Stmt::Block(statements) = &program.statements[0] {
        ...
        // First nested block
        if let Stmt::Block(inner1) = &statements[0] {
            assert_eq!(inner1.len(), 1);
            assert!(matches!(inner1[0], Stmt::VarDeclaration(_, _)));
        } else {
            panic!("Expected first nested Block statement");
        }
        
        // Second nested block
        if let Stmt::Block(inner2) = &statements[1] {
            assert_eq!(inner2.len(), 1);
            assert!(matches!(inner2[0], Stmt::VarDeclaration(_, _)));
        } else {
...
fn test_parse_deeply_nested_blocks() {
    ...
    assert_eq!(program.statements.len(), 1);
    if let Stmt::Block(level1) = &program.statements[0] {
        assert_eq!(level1.len(), 1);
        
        if let Stmt::Block(level2) = &level1[0] {
            assert_eq!(level2.len(), 1);
            
            if let Stmt::Block(level3) = &level2[0] {
                assert_eq!(level3.len(), 1);
                assert!(matches!(level3[0], Stmt::VarDeclaration(_, _)));
            } else {
...
fn test_multiple_separate_blocks() {
    ...
    // First block
    if let Stmt::Block(statements1) = &program.statements[0] {
        assert_eq!(statements1.len(), 1);
        assert!(matches!(statements1[0], Stmt::VarDeclaration(_, _)));
    } else {
        panic!("Expected first Block statement");
    }
    
    // Second block
    if let Stmt::Block(statements2) = &program.statements[1] {
        assert_eq!(statements2.len(), 1);
        assert!(matches!(statements2[0], Stmt::VarDeclaration(_, _)));
    } else {
