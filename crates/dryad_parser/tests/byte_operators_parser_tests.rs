...
mod byte_operators_parser_tests {
    ...
    fn test_parse_binary_number() {
        ...
        match ast {
            Expr::Literal(dryad_parser::ast::Literal::Number(value)) => {
    ...
    fn test_parse_byte_numbers_in_expression() {
        ...
        match ast {
            Expr::Binary(left, op1, right) => {
                ...
                match *left {
                    Expr::Binary(left_left, op2, left_right) => {
                        assert_eq!(op2, "+");
                        // Verifica os números
                        match (*left_left, *left_right) {
                            (Expr::Literal(dryad_parser::ast::Literal::Number(n1)), 
                             Expr::Literal(dryad_parser::ast::Literal::Number(n2))) => {
                ...
                match *right {
                    Expr::Literal(dryad_parser::ast::Literal::Number(n3)) => {
    ...
    fn test_byte_numbers_precedence() {
        ...
        match ast {
            Expr::Binary(left, op, right) => {
                ...
                match (*left, *right) {
                    (Expr::Literal(dryad_parser::ast::Literal::Number(n1)), 
                     Expr::Binary(mult_left, mult_op, mult_right)) => {
