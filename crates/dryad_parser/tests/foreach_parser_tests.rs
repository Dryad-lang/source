...
fn test_foreach_with_array_literal() {
    ...
    match &program.statements[0] {
        Stmt::ForEach(var, iterable, _body) => {
            assert_eq!(var, "item");
            match iterable {
                Expr::Array(elements) => {
...
fn test_foreach_with_variable() {
    ...
    match &program.statements[0] {
        Stmt::ForEach(var, iterable, _body) => {
            assert_eq!(var, "x");
            match iterable {
                Expr::Variable(name) => assert_eq!(name, "lista"),
        ...
        Stmt::ForEach(var, iterable, _body) => {
            ...
            match iterable {
                Expr::Tuple(elements) => {
                    assert_eq!(elements.len(), 3);
                    match &elements[1] {
                        Expr::Literal(Literal::String(s)) => assert_eq!(s, "test"),
...
fn test_nested_foreach() {
    ...
    match &program.statements[0] {
        Stmt::ForEach(var, iterable, body) => {
            assert_eq!(var, "outer");
            match iterable {
                Expr::Variable(name) => assert_eq!(name, "lists"),
                _ => panic!("Expected variable expression"),
            }
            ...
            match body.as_ref() {
                Stmt::Block(statements) => {
                    ...
                    match &statements[0] {
                        Stmt::ForEach(inner_var, inner_iterable, _inner_body) => {
                            assert_eq!(inner_var, "inner");
                            match inner_iterable {
                                Expr::Variable(name) => assert_eq!(name, "outer"),
                                _ => panic!("Expected variable expression"),
                            }
                        },
...
fn test_foreach_vs_traditional_for() {
    ...
    match &program.statements[0] {
        Stmt::For(_init, _condition, _update, _body) => {
...
fn test_foreach_error_missing_braces() {
    ...
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.code(), 2064); // Expected '{' after foreach expression
}
        ...
        Stmt::ForEach(var, iterable, _body) => {
            ...
            match iterable {
                Expr::Index(array, index) => {
                    match array.as_ref() {
                        Expr::Variable(name) => assert_eq!(name, "array"),
                        _ => panic!("Expected variable expression"),
                    }
                    match index.as_ref() {
