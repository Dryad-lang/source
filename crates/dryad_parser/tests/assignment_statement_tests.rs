...
mod assignment_statement_tests {
    ...
    fn test_simple_assignment() {
        ...
        if let Stmt::Assignment(name, expr) = &program.statements[0] {
            assert_eq!(name, "x");
            assert!(matches!(expr, Expr::Literal(Literal::Number(5.0))));
        } else {
    ...
    fn test_subtraction_assignment() {
        ...
        if let Stmt::Assignment(name, expr) = &program.statements[0] {
            ...
            if let Expr::Binary(left, op, right) = expr {
                assert_eq!(op, "-");
                assert!(matches!(**left, Expr::Variable(ref var_name) if var_name == "x"));
                assert!(matches!(**right, Expr::Literal(Literal::Number(3.0))));
            } else {
                panic!("Esperado Binary expression");
            }
        } else {
    ...
    fn test_division_assignment() {
        ...
        if let Stmt::Assignment(name, expr) = &program.statements[0] {
            ...
            if let Expr::Binary(left, op, right) = expr {
                assert_eq!(op, "/");
                assert!(matches!(**left, Expr::Variable(ref var_name) if var_name == "x"));
                assert!(matches!(**right, Expr::Literal(Literal::Number(4.0))));
            } else {
                panic!("Esperado Binary expression");
            }
        } else {
        ...
        if let Stmt::Assignment(name, expr) = &program.statements[0] {
            ...
            if let Expr::Binary(left, op, right) = expr {
                ...
                if let Expr::Binary(right_left, right_op, right_right) = &**right {
                    assert_eq!(right_op, "*");
                    assert!(matches!(**right_left, Expr::Variable(ref var_name) if var_name == "y"));
                    assert!(matches!(**right_right, Expr::Literal(Literal::Number(2.0))));
                } else {
    ...
    fn test_multiple_assignments() {
        ...
        for (i, stmt) in program.statements.iter().enumerate() {
            if let Stmt::Assignment(name, _) = stmt {
                assert_eq!(name, "x");
            } else {
    ...
    fn test_assignment_vs_declaration() {
        ...
        assert!(matches!(program.statements[0], Stmt::VarDeclaration(_, _)));
        
        // Os outros dois devem ser Assignment
        assert!(matches!(program.statements[1], Stmt::Assignment(_, _)));
        assert!(matches!(program.statements[2], Stmt::Assignment(_, _)));
    }
    ...
    fn test_assignment_with_variables() {
        ...
        if let Stmt::Assignment(name, expr) = &program.statements[0] {
            ...
            if let Expr::Binary(left, op, right) = expr {
                assert_eq!(op, "+");
                assert!(matches!(**left, Expr::Variable(ref var_name) if var_name == "y"));
                assert!(matches!(**right, Expr::Variable(ref var_name) if var_name == "z"));
            } else {
                panic!("Esperado Binary expression");
            }
        } else {
    ...
    fn test_chained_assignment_operations() {
        ...
        if let Stmt::VarDeclaration(name, _) = &program.statements[0] {
        ...
        for (i, expected_op) in expected_ops.iter().enumerate() {
            if let Stmt::Assignment(name, expr) = &program.statements[i + 1] {
                assert_eq!(name, "total");
