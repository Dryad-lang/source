...
mod parser_tests {
    ...
    fn test_parse_boolean_false() {
        ...
        match expr {
            Expr::Literal(Literal::Bool(b)) => assert_eq!(b, false),
            _ => panic!("Esperado boolean literal"),
        }
    }
    ...
    fn test_parse_variable() {
        ...
        match expr {
            Expr::Variable(name) => assert_eq!(name, "variable_name"),
    ...
    fn test_parse_addition() {
        ...
        match expr {
            Expr::Binary(left, op, right) => {
                assert_eq!(op, "+");
                match (*left, *right) {
                    (Expr::Literal(Literal::Number(2.0)), Expr::Literal(Literal::Number(3.0))) => {},
    ...
    fn test_parse_subtraction() {
        ...
        match expr {
            Expr::Binary(left, op, right) => {
                assert_eq!(op, "-");
                match (*left, *right) {
                    (Expr::Literal(Literal::Number(5.0)), Expr::Literal(Literal::Number(2.0))) => {},
                    _ => panic!("Operandos incorretos"),
                }
            },
            _ => panic!("Esperado expressão binária"),
    ...
    fn test_parse_multiplication() {
        ...
        match expr {
            Expr::Binary(left, op, right) => {
                assert_eq!(op, "*");
                match (*left, *right) {
                    (Expr::Literal(Literal::Number(3.0)), Expr::Literal(Literal::Number(4.0))) => {},
                    _ => panic!("Operandos incorretos"),
                }
            },
            _ => panic!("Esperado expressão binária"),
    ...
    fn test_parse_division() {
        ...
        match expr {
            Expr::Binary(left, op, right) => {
                assert_eq!(op, "/");
                match (*left, *right) {
                    (Expr::Literal(Literal::Number(8.0)), Expr::Literal(Literal::Number(2.0))) => {},
                    _ => panic!("Operandos incorretos"),
                }
            },
            _ => panic!("Esperado expressão binária"),
    ...
    fn test_operator_precedence() {
        ...
        match expr {
            Expr::Binary(left, op, right) => {
                ...
                match *right {
                    Expr::Binary(_, mult_op, _) => assert_eq!(mult_op, "*"),
    ...
    fn test_parse_equality() {
        ...
        match expr {
            Expr::Binary(_, op, _) => assert_eq!(op, "=="),
    ...
    fn test_parse_parentheses() {
        ...
        match expr {
            Expr::Binary(left, op, right) => {
                ...
                match *left {
                    Expr::Binary(_, add_op, _) => assert_eq!(add_op, "+"),
    ...
    fn test_unexpected_token_error() {
        ...
        match result.unwrap_err() {
            DryadError::Parser(2001, _) => {}, // E2001 - Unexpected Token
    ...
    fn test_missing_closing_parenthesis() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            DryadError::Parser(2005, _) => {}, // E2005 - Missing Closing Parenthesis
    ...
    fn test_string_concatenation_precedence() {
        ...
        match expr {
            Expr::Binary(left, op, right) => {
                assert_eq!(op, "+");
                match *left {
                    Expr::Binary(_, inner_op, _) => assert_eq!(inner_op, "+"),
