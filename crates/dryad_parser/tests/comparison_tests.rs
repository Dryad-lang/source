...
mod comparison_tests {
    ...
    fn test_equality_operator() {
        ...
        if let Stmt::VarDeclaration(name, Some(Expr::Binary(left, op, right))) = &program.statements[0] {
            assert_eq!(name, "igual");
            assert_eq!(op, "==");
            assert!(matches!(**left, Expr::Literal(Literal::Number(5.0))));
            assert!(matches!(**right, Expr::Literal(Literal::Number(10.0))));
        } else {
    ...
    fn test_complex_comparison_with_variables() {
        ...
        if let Stmt::VarDeclaration(name, Some(Expr::Literal(Literal::Number(5.0)))) = &program.statements[0] {
            assert_eq!(name, "x");
        } else {
        ...
        if let Stmt::VarDeclaration(name, Some(Expr::Literal(Literal::Number(10.0)))) = &program.statements[1] {
            assert_eq!(name, "y");
        } else {
        ...
        // Verifica comparação
        if let Stmt::VarDeclaration(name, Some(Expr::Binary(left, op, right))) = &program.statements[2] {
            assert_eq!(name, "resultado");
            assert_eq!(op, "<");
            assert!(matches!(**left, Expr::Variable(ref var_name) if var_name == "x"));
            assert!(matches!(**right, Expr::Variable(ref var_name) if var_name == "y"));
        } else {
            panic!("Esperado VarDeclaration com Binary expression");
        }
    }
    ...
    fn test_string_comparison() {
        ...
        if let Stmt::VarDeclaration(name, Some(Expr::Binary(left, op, right))) = &program.statements[0] {
            assert_eq!(name, "igual");
            assert_eq!(op, "==");
            assert!(matches!(**left, Expr::Literal(Literal::String(ref s)) if s == "hello"));
            assert!(matches!(**right, Expr::Literal(Literal::String(ref s)) if s == "world"));
        } else {
            panic!("Esperado VarDeclaration com Binary expression");
        }
    }
    ...
    fn test_null_comparison() {
        ...
        if let Stmt::VarDeclaration(name, Some(Expr::Binary(left, op, right))) = &program.statements[0] {
            assert_eq!(name, "igual");
            assert_eq!(op, "==");
            assert!(matches!(**left, Expr::Literal(Literal::Null)));
            assert!(matches!(**right, Expr::Literal(Literal::Null)));
        } else {
            panic!("Esperado VarDeclaration com Binary expression");
        }
    }
    ...
    fn test_chained_comparisons() {
        ...
        if let Stmt::VarDeclaration(name, Some(Expr::Binary(left, op, right))) = &program.statements[0] {
            ...
            if let Expr::Binary(left_left, left_op, left_right) = &**left {
                assert_eq!(left_op, "<");
                assert!(matches!(**left_left, Expr::Literal(Literal::Number(1.0))));
                assert!(matches!(**left_right, Expr::Literal(Literal::Number(2.0))));
            } else {
                panic!("Esperado Binary expression no lado esquerdo");
            }

            // Right side should be 2 < 3
            if let Expr::Binary(right_left, right_op, right_right) = &**right {
                assert_eq!(right_op, "<");
                assert!(matches!(**right_left, Expr::Literal(Literal::Number(2.0))));
                assert!(matches!(**right_right, Expr::Literal(Literal::Number(3.0))));
            } else {
