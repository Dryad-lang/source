...
fn test_class_declaration_parsing() {
    ...
    if let Ok(program) = result {
        ...
        if let Stmt::ClassDeclaration(name, parent, members) = &program.statements[0] {
...
fn test_class_with_inheritance() {
    ...
    if let Ok(program) = result {
        if let Stmt::ClassDeclaration(name, parent, _) = &program.statements[0] {
...
fn test_class_with_visibility_modifiers() {
    ...
    if let Ok(program) = result {
        if let Stmt::ClassDeclaration(_, _, members) = &program.statements[0] {
